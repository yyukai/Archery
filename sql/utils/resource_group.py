...
def user_instances(user, type=None, db_type=None, tag_codes=list()):
    """
    获取用户实例列表（通过资源组间接关联）
    :param user:
    ...
    if user.has_perm('sql.query_all_instances'):
        ...
        # 先获取用户关联的资源组
        resource_groups = user.resource_group.all()
        # 再获取实例
...
def auth_group_users(auth_group_names, group_id):
    ...
    """
    # 获取资源组关联的用户
    users = ResourceGroup.objects.get(group_id=group_id).res_group_users.all()
    # 过滤在该权限组中的用户
