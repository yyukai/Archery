...
def associated_objects(request):
    ...
    rows_users = resource_group.res_group_users.all()
    rows_instances = resource_group.res_group_instances.all()
    # 过滤搜索
    if search:
    ...
    rows_instances = [{
        "object_id": i.id,
        "object_type": 1,
        "object_name": i.instance_name,
        "group_id": group_id,
        "group_name": resource_group.group_name
    } for i in rows_instances]
    # 过滤对象类型
    if object_type == '0':
        rows_obj = rows_users
        count = len(rows_obj)
...
def unassociated_objects(request):
    ...
    if object_type == 0:
        ...
        associated_instance_ids = [ins.id for res_group in ResourceGroup.objects.filter(group_id=group_id)
                                   for ins in res_group.res_group_instances.all()]
...
def instances(request):
    ...
    # 先获取资源组关联所有实例列表
    instances = ResourceGroup.objects.get(group_id=group_id).res_group_instances.all()
    ...
    try:
        if object_type == '0':  # 用户
            for obj in object_list:
                user_id = int(obj.split(',')[0])
                Users.objects.get(id=user_id).resource_group.add(resource_group)
        elif object_type == '1':  # 实例
            for obj in object_list:
                ins_id = int(obj.split(',')[0])
                Instance.objects.get(id=ins_id).resource_group.add(resource_group)
